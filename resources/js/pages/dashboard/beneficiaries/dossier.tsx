        ...
        <DashboardLayout title={`Dossier — ${b.full_name}`} subtitle="">
            ...
            <div className="mb-5 rounded-2xl border border-warning-200 bg-warning-50 p-4">
                <div className="flex items-start gap-3">
                    <svg className="size-5 shrink-0 text-warning-600" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                        <path
                            strokeLinecap="round"
            ...
            <div className="mt-5">
                <Link href={`/beneficiaries/${b.id}`} className="text-sm text-ink-500 hover:text-brand-600">
...
function Section({ label, content }: { label: string; content: string | null }) {
    return (
        <div className="border-b border-ink-100 py-3 last:border-b-0">
            <h4 className="text-[11px] font-semibold uppercase tracking-wider text-ink-500">{label}</h4>
            <p className={`mt-1.5 text-sm leading-relaxed ${content ? 'text-ink-700' : 'italic text-ink-400'}`}>
