...
1. **NotifyReferentRhJob mail wiring incomplete** (Concern 7/8, **YELLOW**): `app/Jobs/NotifyReferentRhJob.php:35` has TODO "route to actual referent_rh users via Notification facade once mail templates land." Right now the job only logs. Same shape as the cert-expiry slice that just shipped (`CertificationExpiringNotification`). Fix: build `WeakSignalDetectedNotification` mail class, send to all RH users in the signal's structure. ~1 hour of work, follows the cert-expiry template exactly.

2. **Sub-entity policies missing** (Concern 2, **YELLOW** — defensible but worth a decision): `AuditGridItem`, `DiscussionGroupMember`, `IncidentActionCorrective`, `IncidentSuivi`, `InterventionCompletedTask`, `InterventionPhoto` have no `app/Policies/*Policy.php` files. They are currently authorized via their parent aggregate's policy (e.g. AuditGridItem checked through AuditGridPolicy; InterventionPhoto through InterventionPolicy). This is a deliberate aggregate-root pattern, not an oversight — but if any controller/route binds directly to one of these (e.g. `DELETE /api/v1/intervention-photos/{photo}`), the absence of an explicit policy + BasePolicy::before() guard is a real hole. Audit each route file to confirm no direct sub-entity bindings, OR write the six policies. Recommendation: write the six policies; the cost is low and the defense-in-depth payoff is high.

3. **PAC state-change application logging** (Concern 8, **YELLOW** — informational gap, not security): `PacActionObserver` and `PacObserver` only flush dashboard cache; they don't `Log::info()` state changes. owen-it/laravel-auditing on `PacAction` and `Pac` models DOES capture the change in the `audits` table (the formal trail), but there's no parallel application log entry like the cert-expiry job emits. Decide whether the audit table alone is sufficient (it is for compliance) or whether human-readable log lines are wanted for ops dashboards. Either choice is defensible.
