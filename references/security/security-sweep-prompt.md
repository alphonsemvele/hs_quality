# Comprehensive Security Sweep Prompt

Reusable adversarial security audit prompt — paste into a Claude Code subagent
(or any LLM red-team session) when you want a deep, paranoid review of the
codebase.

When to run:
- Before every external pilot release
- After a major domain lands (e.g. a new module is merged to main)
- Quarterly even when nothing major has shipped
- Before any HDS / pentest engagement, to catch low-hanging fruit first

How to run inside this repo:
- `claude /security-review` covers pending changes only
- For a full-codebase sweep, paste the prompt below into a fresh agent session
