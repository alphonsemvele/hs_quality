...
- **Never copy a workstation `.env` to a server.** Production env vars come from the deployment platform (Forge, Vapor, Render, K8s secret, etc.) — never from a file an engineer typed by hand.
- **`APP_DEBUG=true` is local-only.** `AppServiceProvider::guardProductionDebug()` aborts boot if `APP_ENV=production && APP_DEBUG=true`. If you ever see that error in production, check whoever pushed the env config.
- **`APP_ENV` for staging is `staging`** (not `production`). `APP_DEBUG` MAY be true in staging if the team needs detailed errors during pre-prod testing — but turn it off again before any external pilot user gets a staging URL.
- **Sentry DSN is a public client identifier**, not a secret. Don't bother rotating it unless you see signal poisoning. Real secrets (AWS keys, DB passwords, Reverb app secret, Sanctum/`APP_KEY`) MUST come from the deployment platform's secret store, never from a committed file.
- **PHP `gd` (or `imagick`) extension is REQUIRED on every server that handles photo uploads.** `InterventionMediaService` re-encodes uploaded photos through Intervention Image to strip EXIF metadata and embedded payloads (Wave 1 / H8). Install with `apt-get install php-gd` (Debian/Ubuntu), `apk add php83-gd` (Alpine), or via your Docker base image. Without it, photo upload throws.
- **Pre-commit hooks via lefthook** (Block A / #58) — config in `lefthook.yml`. Every clone runs once: `npm install --save-dev lefthook && npx lefthook install`. Hooks: Pint + PHP syntax check + named-individual leak guard on pre-commit; `composer audit` + full Pest suite on pre-push. Bypass with `git commit --no-verify` ONLY for emergencies.
...
- **Security sweep prompt**: [references/security/security-sweep-prompt.md](references/security/security-sweep-prompt.md) — paste into a fresh agent for a full-codebase adversarial audit. Run before every external pilot release, after a major domain merges, and quarterly even when nothing major has shipped. `/security-review` covers pending changes only.
