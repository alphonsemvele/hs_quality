...
| M4.17 | Web controllers + Inertia pages (group chat, news feed, document library, forum) | [~] | Frontend pages exist (merged from `origin/feature/front` 2026-05-04) — `resources/js/pages/dashboard/communication/index.tsx` (141 LOC, real Inertia page with messages/channels/documents UI). Backend `CommunicationController` is a stub returning hard-coded demo fixtures — needs real wiring to `MessageService` / `NewsFeedService` / `DocumentLibraryService` / `QaService` + Form Request validation. Wiring scheduled with M4.18 (API endpoints share the same service calls — controllers can be built in lockstep). |
